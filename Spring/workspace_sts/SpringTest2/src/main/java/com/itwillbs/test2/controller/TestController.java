...
//	}
	...
	// 기본값을 설정할 수도 있다!
	// => @RequestParam 어노테이션에 defaultValue 속성을 추가하여 기본값 설정 가능
	// ex) @RequestParam(defaultValue = "기본값") 변수선언
	@RequestMapping(value = "/dispatcher", method = RequestMethod.GET)
	public String dispatcher(Model model, @RequestParam String name, @RequestParam(defaultValue = "0") int age) throws UnsupportedEncodingException {
		System.out.println("이름 : " + name);
		System.out.println("나이 : " + age); // age 파라미터 없으면 콘솔에 0 출력됨
		
		// 만약, age 파라미터를 URL 에 전달하지 않은 채로
		// 기본값으로 적용된 age 값을 redirect.jsp 페이지까지 사용하기 위해서는
		// Model 객체를 사용하여 저장 후 포워딩 수행 가능
		// => dispatcher() 메서드 파라미터로 Model 타입 변수 선언 필수!
	...
	// ModelAndView : 데이터를 저장하는 Model 객체 관리와 View 페이지 포워딩을 함께 수행하는 객체
	// => 메서드 정의 시 리턴타입을 ModelAndView 타입으로 설정
	@RequestMapping(value = "/mav", method = RequestMethod.GET)
	public ModelAndView modelAndView() {
		// ModelAndView 객체 사용 시 데이터 저장에 필요한 객체로 HashMap 객체 활용
		Map<String, TestVO> map = new HashMap<String, TestVO>();
		map.put("key", new TestVO("제목", "내용"));
		
		// Model 객체를 사용하여 데이터를 저장하는 경우 
		// return 문 뒤에 ModelAndView 객체를 생성할 때 포워딩 할 뷰페이지 명만 지정하면 되지만
		// Model 객체 외의 다른 객체를 뷰페이지로 전송하려면
		// return new ModelAndView("뷰페이지명", "전송할 객체의 이름", 전송할 객체);
		return new ModelAndView("model_and_view", "map", map); // Dispatcher 방식으로 포워딩
	}
